...
if [ -f $VOLUME/ts3server.sqlitedb ]
  then
	...
	ln -s $VOLUME/ts3server.sqlitedb /home/teamspeak/teamspeak3-server_linux-amd64/ts3server.sqlitedb 
fi
echo "2. Link the files-folder into the host-mounted volume."
if [ -f $VOLUME/files ]
  then
    echo "Linking files-folder."
	ln -s $VOLUME/files /home/teamspeak/teamspeak3-server_linux-amd64/files
fi
...
if [ -f $VOLUME/ts3server.ini ]
  then
	...
	/home/teamspeak/teamspeak3-server_linux-amd64/ts3server_minimal_runscript.sh \
